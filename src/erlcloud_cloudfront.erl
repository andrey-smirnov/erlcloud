...
-export([list_distributions_all/0, list_distributions_all/1,
...
-spec list_distributions_all() -> ok_error(proplist()).
list_distributions_all() ->
    list_distributions_all(erlcloud_aws:default_config()).


-spec list_distributions_all(aws_config()) -> ok_error(proplist()).
list_distributions_all(Config) ->
    case list_distributions(?MAX_RESULTS, undefined, Config) of
        {ok, Res, NextMarker} ->
            list_distributions_acc(Res, NextMarker, Config);
        E -> 
            E
    end.


list_distributions_acc(Res, undefined, _Config) ->
    {ok, Res};
list_distributions_acc(Res, Marker, Config) ->
    case list_distributions(?MAX_RESULTS, Marker, Config) of
        {ok, NewRes, NextMarker} ->
            list_distributions_acc(Res ++ NewRes, NextMarker, Config);
        E -> 
            E
    end.
...
list_distributions(MaxResults, Marker, Config) when
    ...
    Params = [{"MaxItems", MaxResults}] ++ case Marker of
                                               undefined -> [];
                                               M -> [{"Marker", M}]
                                           end,
